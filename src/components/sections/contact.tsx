...
export function ContactSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Map form data to Payload CMS contact schema
      const contactData = {
        name: formData.name,
        email: formData.email,
        phone: formData.phone || undefined,
        subject: `Booking Inquiry - ${selectedPerformanceType === 'both' ? 'Performance Type TBD' : selectedPerformanceType.charAt(0).toUpperCase() + selectedPerformanceType.slice(1)} Performance`,
        message: `Event Details:
- Date: ${formData.eventDate || 'To be determined'}
- Venue: ${formData.venue || 'Not specified'}
- Expected Guests: ${formData.expectedGuests || 'Not specified'}
- Performance Type: ${selectedPerformanceType === 'both' ? 'Need advice on performance type' : selectedPerformanceType}
...
${formData.message || 'No additional message provided'}`,
      ...
      setSubmitMessage('Thank you! Your booking inquiry has been sent successfully. We\'ll get back to you within 24 hours.');
      ...
      setSelectedPerformanceType("both");
      
    } catch (error) {
      console.error('Contact form submission error:', error);
      setSubmitStatus('error');
      setSubmitMessage('Sorry, there was an error sending your message. Please try again or contact us directly at booking@thedutchqueen.com');
    } finally {
          ...
          >
            ...
            <motion.div variants={fadeInUp}>
              ...
              <div className="flex space-x-4">
                ...
                <a href={socialLinks.instagram} target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-blue-800 rounded-full flex items-center justify-center hover:bg-blue-700 transition-colors">
                  <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M22.46 6c-.77.35-1.6.58-2.46.69.88-.53 1.56-1.37 1.88-2.38-.83.5-1.75.85-2.72 1.05C18.37 4.5 17.26 4 16 4c-2.35 0-4.27 1.92-4.27 4.29 0 .34.04.67.11.98C8.28 9.09 5.11 7.38 3 4.79c-.37.63-.58 1.37-.58 2.15 0 1.49.75 2.81 1.91 3.56-.71 0-1.37-.2-1.95-.5v.03c0 2.08 1.48 3.82 3.44 4.21a4.22 4.22 0 0 1-1.93.07 4.28 4.28 0 0 0 4 2.98 8.521 8.521 0 0 1-5.33 1.84c-.34 0-.68-.02-1.02-.06C3.44 20.29 5.7 21 8.12 21 16 21 20.33 14.46 20.33 8.79c0-.19 0-.37-.01-.56.84-.6 1.56-1.36 2.14-2.23z"/>
                  </svg>
                </a>
                <a href={socialLinks.spotify} target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-pink-600 rounded-full flex items-center justify-center hover:bg-pink-500 transition-colors">
                  <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24">
                ...
                <a href={socialLinks.youtube} target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-red-600 rounded-full flex items-center justify-center hover:bg-red-500 transition-colors">
                  <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24">
